...
const fetch = require("node-fetch"); // вместо импорта
...
const app = express(); // экземпляр экспреса
const router = express.Router(); // для манипуляций с ручками
const port = 3027;
...
app.use(express.json()); // включаем поддержку json в api
app.use(express.static(path.join(__dirname, "../public"))); // пусть до файлов
...
const authUrls = ["/auth/logout"];

router.use(async function (req, res, next) {
  // получаем jwt-токен и проверяем
  let status = 200;
...
app.post(baseUrl + "auth/updatePassword", async (req, res) => {
  const result = await authApi.updatePassword(
    _getBearerTokenFromRequest(req),
    req.body
  );
  _wrapResponse(res, result);
});
...
app.get(baseUrl + "auth/favourites", async (req, res) => {
  const result = await collectionApi.getFavouriteCollectionItemsByOwner(
    _getBearerTokenFromRequest(req)
  );
  res.send(result);
});
...
app.post(baseUrl + "auth/favourites", async (req, res) => {
  const result = await favouriteApi.favouritesAdd(
    _getBearerTokenFromRequest(req),
    req.body
  );
  _wrapResponse(res, result);
...
app.delete(baseUrl + "auth/favourites", async (req, res) => {
  const result = await favouriteApi.favouritesRemove(
    _getBearerTokenFromRequest(req),
    req.body
  );
  _wrapResponse(res, result);
...
app.get(baseUrl + "auth/myNft", async (req, res) => {
  const result = await collectionApi.getCollectionItemsByOwner(
    _getBearerTokenFromRequest(req)
  );
  res.send(result);
});
...
app.get(baseUrl + "topSales/:days", async (req, res) => {
  const days = req.params.days;
  const result = await collectionApi.topSales(
    _getBearerTokenFromRequest(req),
    days
  );
  res.send(result);
...
app.get(baseUrl + "collection/:contractAddress/all", async (req, res) => {
  ...
  const result = await collectionApi.getCollectionItems(
    _getBearerTokenFromRequest(req),
    contractAddress,
    page,
    size,
...
app.get(
  baseUrl + "collection/:contractAddress/item/:tokenId",
  async (req, res) => {
    const contractAddress = req.params.contractAddress.toLowerCase();
    const tokenId = req.params.tokenId;
    const result = await collectionApi.getCollectionItem(
      _getBearerTokenFromRequest(req),
      contractAddress,
      tokenId
    );
