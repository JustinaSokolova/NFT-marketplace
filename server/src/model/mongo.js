...
const mongoUrl = 'mongodb://navyuser:jhassct872hbJGFJgkcva2s@23.111.202.19:27017/?authMechanism=DEFAULT';
...
async function getActiveProjects() {
    const projects = await projectSchema.model.find({
        projectState: {
            "$ne": projectSchema.ProjectState.DISABLED
        }
    });
    return projects;
}
...
async function getMintByCollection(collection) {
    return await mintSchema.model.findOne({ _id: collection.mint }).select(['-_id', '-__v']);
}
